...
- 运营动作：红娘创建后默认为 `pending`；审核通过改为 `upcoming`；拒绝改为 `rejected` 并填写 `reviewRemark`。
...
### 会员添加红娘申请排查 `hl_member_matchmaker_requests`
...
- 小程序端提交红娘申请后，检查 `hl_matchmakers.certificationStatus = 0`。
- 后台通过红娘认证后，确认红娘端可进入会员、资源、沙龙管理页面。
- 红娘创建沙龙后，确认新活动为 `pending`，用户端不可报名。
- 后台通过沙龙审核后，确认用户端活动列表可见且可报名。
- 用户报名和取消报名后，检查 `hl_registrations` 与 `hl_salon_events.currentParticipants` 一致。
- 会员申请添加红娘时，确认只有红娘端审批会同步写入 `hl_members` 和 `hl_messages`。
