...
本文档用于指导运营人员在 CloudBase 云后台/CMS 中处理小程序日常审批工作，包括主理人审批、沙龙审批、会员资料维护、报名查看和会员添加主理人申请排查。
...
| 分组 | 菜单 | 数据模型 |
| --- | --- | --- |
| 报名与排查 | 会员添加主理人申请排查 | hl_member_matchmaker_requests |
| 报名与排查 | 报名管理 | hl_registrations |
| 会员管理 | 会员服务管理 | hl_members |
| 会员管理 | 会员资料管理 | hl_profiles |
| 审批管理 | 沙龙审批 | hl_salon_events |
| 审批管理 | 主理人审批 | hl_matchmakers |

## 2. 审批总原则

1. 可以在后台数据库中处理的内容：主理人认证状态、沙龙审核状态、会员展示资料、服务等级、备注、报名签到等轻量维护。
...
| --- | --- | --- |
| 0 | 待审核 | 主理人端显示等待审核 |
| 1 | 已拒绝 | 主理人端显示拒绝原因，可重新提交 |
| 2 | 已通过 | 主理人可进入会员、资源、沙龙管理 |
...
4. 确认 `status` 为 `1`。
5. 如需设置主理人等级，调整 `level`。
6. 保存。

审批通过后，应让主理人重新进入小程序，确认可以打开：
...
| --- | --- |
| id | 活动业务主键 |
| organizerId | 活动创建人，改错会导致主理人看不到自己的活动 |
...
2. 不建议在后台手工新增会员关系。
3. 会员申请添加主理人、主理人审批会员申请，必须走小程序端流程。
...
```text
主理人审批 -> 筛选 certificationStatus = 0 -> 编辑 -> certificationStatus = 2 -> 保存
```

验证：

```text
主理人重新进入小程序 -> 可以打开主理人工作台和沙龙管理
```

### 9.2 主理人认证拒绝

```text
主理人审批 -> 编辑 -> certificationStatus = 1 -> certificationRemark 填写原因 -> 保存
```

验证：

```text
主理人端看到拒绝原因，并可以重新提交申请
```
...
2. 确认是否改了正确数据模型。
3. 确认状态值是否填写正确，例如主理人通过必须是数字 `2`，沙龙通过必须是文本 `upcoming`。
